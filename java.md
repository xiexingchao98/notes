...
对象的 finalize 方法不一定会执行。

垃圾回收器只有在内存不足时才会工作。

finalize 函数优先级远低于其它线程，不保证一定执行。

对象第一次进入 GC ，已执行 finalize 并进入 F-Queue 队列，但是此时有新的引用指向该对象，则停止回收。接着下次该对象又进入了 GC 范围，此时因为 finalize 已经执行过一次，故不会再执行而是直接回收。
...
+ 单例模式（懒汉构造指的是实例在第一次使用时被构建，饿汉构造指的是在类装载时构造）
+ 工厂模式
+ 原型模式
...
synchronized 修饰静态方法锁定该类对象，修饰对象方法锁定该对象。	
...
静态内部类：可以不依赖外部类实例化，只有静态内部类才可以使用静态方法
...
1. 在其它类中使用 `className.propertyName`   
2. 在本类中可直接使用属性名
...
复制算法：将 A 区域中继续存活的对象转移至 B 区域

标记清理算法：在区域内标记可达对象，然后回收不可达对象，此时会出现碎片
...
if 条件中只能存在布尔值，不能存在整数值，使用整数作为调件会引发异常
